module dexlyn::vault {
    ...
    public (friend) fun deposit_vault<VaultT, AssetT>(_coin: Coin<AssetT>) acquires Vault {
        let vault = borrow_global_mut<Vault<VaultT, AssetT>>(@dexlyn);
    ...
    /// Test initialize
    fun T_register(host: &signer, supra_framework: &signer) {
        timestamp::set_time_has_started_for_testing(supra_framework);
        supra_coin::ensure_initialized_with_apt_fa_metadata_for_test();
        if (!account::exists_at(address_of(host))) {
            supra_account::create_account(address_of(host));
    ...
    #[test(host = @0xC0FFEE, supra_framework = @supra_framework)]
    #[expected_failure(abort_code = E_NOT_AUTHORIZED, location = Self)]
    /// Test initialize
    fun T_register_not_allowed(host: &signer, supra_framework: &signer){
        timestamp::set_time_has_started_for_testing(supra_framework);
        supra_coin::ensure_initialized_with_apt_fa_metadata_for_test();
        if (!account::exists_at(address_of(host))) {
            supra_account::create_account(address_of(host));
    ...
    #[test(host = @dexlyn, supra_framework = @supra_framework)]
    /// Test initialize
    fun T_deposit(host: &signer, supra_framework: &signer) acquires Vault {
    ...
    #[test(host = @dexlyn, supra_framework = @supra_framework)]
    /// Test initialize
    fun T_withdraw(host: &signer, supra_framework: &signer) acquires Vault {
        call_test_setting(host, supra_framework);
        let deposit_coin = coin::withdraw<TEST_USDC>(host, 10000);
        deposit_vault<vault_type::CollateralVault, TEST_USDC>(deposit_coin);
