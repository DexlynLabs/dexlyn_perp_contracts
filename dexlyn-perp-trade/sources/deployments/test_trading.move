module dexlyn::test_trading {
    ...
    /// whole events in faucet for dexlyn perp
    ...
    public entry fun set_limit_faucet<T>(host: &signer, _limit_faucet: bool) acquires TEST_USDC_INFO {
        ...
        assert!(host_addr == @dexlyn, E_NOT_ALLOWED);
    ...
    public entry fun faucet_coin<T>(host: &signer, amount: u64) acquires TEST_USDC_INFO {
        let host_addr = signer::address_of(host);
        let info = borrow_global_mut<TEST_USDC_INFO<T>>(@dexlyn);
        supra_account::deposit_coins(host_addr, coin::mint<T>(amount, &info.mint_cap));
    }
    ...
    #[test(host = @dexlyn, supra_framework = @supra_framework, user = @0xC0FFEE)]
    /// faucet tests
    fun T_faucet_test(host: &signer, supra_framework: &signer, user: &signer) acquires TEST_USDC_INFO {
        timestamp::set_time_has_started_for_testing(supra_framework);
        supra_coin::ensure_initialized_with_apt_fa_metadata_for_test();
        supra_account::create_account(signer::address_of(host));
        supra_account::create_account(signer::address_of(user));
        init_module(host);
        let host_addr = signer::address_of(host);
    ...
    /// faucet tests
    fun T_E_NOT_ALLOWED_set_limit_faucet(host: &signer, supra_framework: &signer, user: &signer) acquires TEST_USDC_INFO {
        timestamp::set_time_has_started_for_testing(supra_framework);
        supra_coin::ensure_initialized_with_apt_fa_metadata_for_test();
        supra_account::create_account(signer::address_of(host));
        supra_account::create_account(signer::address_of(user));
    ...
    #[expected_failure(abort_code = E_NOT_ALLOWED, location = Self)]
    /// faucet tests
    fun T_E_NOT_ALLOWED_set_limit_faucet_amount(host: &signer, supra_framework: &signer, user: &signer) acquires TEST_USDC_INFO {
        timestamp::set_time_has_started_for_testing(supra_framework);
        supra_coin::ensure_initialized_with_apt_fa_metadata_for_test();
        supra_account::create_account(signer::address_of(host));
        supra_account::create_account(signer::address_of(user));
