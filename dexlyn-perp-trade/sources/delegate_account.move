module dexlyn::delegate_account {
    ...
    public fun register<AssetT>(_host: &signer, _delegate_account: address) acquires DelegateAccount {
        ...
        if (!account::exists_at(_delegate_account)) {
            supra_account::create_account(_delegate_account);
        };
    }
    ...
    public fun withdraw<AssetT>(_host: &signer, _amount: u64) acquires DelegateAccount {
        let user_address = address_of(_host);
        let delegate_account = borrow_global_mut<DelegateAccount<AssetT>>(user_address);
    ...
    #[test(host = @dexlyn, supra_framework = @supra_framework, delegate_account = @0xC0FFEE)]
    fun T_register(host: &signer, supra_framework: &signer, delegate_account: &signer) acquires DelegateAccount {
        call_test_setting(host, supra_framework);
    ...
    #[test(host = @dexlyn, supra_framework = @supra_framework, delegate_account = @0xC0FFEE)]
    fun T_unregister(host: &signer, supra_framework: &signer, delegate_account: &signer)
    acquires DelegateAccount {
        call_test_setting(host, supra_framework);
        register<TEST_USDC>(host, address_of(delegate_account));
        assert!(is_active<TEST_USDC>(address_of(host)) == true, 0);
    ...
    #[test(host = @dexlyn, supra_framework = @supra_framework, delegate_account = @0xC0FFEE)]
    fun T_deposit(host: &signer, supra_framework: &signer, delegate_account: &signer)
    acquires DelegateAccount {
        call_test_setting(host, supra_framework);
        assert!(is_active<TEST_USDC>(address_of(host)) == false, 0);
        assert!(is_registered<TEST_USDC>(address_of(host), address_of(delegate_account)) == false, 0);
    ...
    #[test(host = @dexlyn, supra_framework = @supra_framework, delegate_account = @0xC0FFEE)]
    fun T_withdraw(host: &signer, supra_framework: &signer, delegate_account: &signer)
    acquires DelegateAccount {
        call_test_setting(host, supra_framework);
    ...
    #[test(host = @dexlyn, supra_framework = @supra_framework, delegate_account = @0xC0FFEE, delegate_account2 = @0xCAFE)]
    fun T_register_multiple_addresses(host: &signer, supra_framework: &signer, delegate_account: &signer, delegate_account2: &signer)
    acquires DelegateAccount {
        call_test_setting(host, supra_framework);
        register<TEST_USDC>(host, address_of(delegate_account));
    ...
    fun T_register_too_many_registered(host: &signer, supra_framework: &signer)
    acquires DelegateAccount {
        call_test_setting(host, supra_framework);
