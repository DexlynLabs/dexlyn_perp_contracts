module dexlyn::managed_price_oracle {
    ...
    public entry fun initialize_module(_host: &signer) {
        price_oracle::initialize_module(_host);
    }
    ...
    public entry fun claim_allowed_update(user: &signer) {
        price_oracle::claim_allowed_update(user);
    }
    ...
    public entry fun set_update_supra_enabled<PairType>(host: &signer, check_supra_enabled: bool) {
        price_oracle::set_update_supra_enabled<PairType>(host, check_supra_enabled);
    }

    public entry fun set_supra_price_identifier<PairType>(host: &signer, supra_price_identifier: u32) {
        price_oracle::set_supra_price_identifier<PairType>(host, supra_price_identifier);
    }

    public entry fun deposit_apt(_host: &signer, _amount: u64) {
        price_oracle::deposit_apt(_host, _amount);
    }
}