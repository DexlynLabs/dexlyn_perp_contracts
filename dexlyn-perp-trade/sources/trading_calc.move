module dexlyn::trading_calc {

    // <-- CONSTANT ----->
    use dexlyn::safe_math::{diff, safe_mul_div, signed_plus, min};
    ...
    public fun calculate_price_impact(
        ...
        let market_skew = diff(_long_open_interest, _short_open_interest);
        ...
        if ((_is_long && _is_increase) || (!_is_long && !_is_increase)) {
            if (market_skew_positive) {
                ...
                after_market_skew = diff(market_skew, _size_delta);
    ...
    public fun calculate_maker_taker_fee(
        ...
        if (market_skew_positive == after_market_skew_positive) {
            ...
            let flipped_taker_rate = safe_mul_div(diff(_size_delta, market_skew), PRECISION,  _size_delta);
