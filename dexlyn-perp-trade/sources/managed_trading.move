module dexlyn::managed_trading {
    ...
    use dexlyn::trading;
    use dexlyn::trading::{ExecuteCapability, AdminCapability, ExecuteCapabilityV2, CapabilityProvider};
    use dexlyn::price_oracle;

    #[test_only]
    use supra_framework::timestamp::set_time_has_started_for_testing;
    #[test_only]
    use supra_framework::account::create_account_for_test;
    #[test_only]
    use dexlyn::pair_types::BTC_USD;
    ...
    public fun set_address_executor_candidate_v2<CollateralType>(
        ...
        let candidates = borrow_global_mut<ExecuteCapabilityCandidateV2<CollateralType>>(@dexlyn);
        vector::push_back(&mut candidates.execute_cap_candidate, candidate);
        vector::push_back(&mut candidates.execute_caps, execute_cap);
    }
    ...
    public entry fun claim_executor_cap<PairType, CollateralType>(
        ...
        let candidate =
            borrow_global_mut<ExecuteCapabilityCandidate<PairType, CollateralType>>(@dexlyn);
    ...
    public entry fun claim_executor_cap_v2<CollateralType>(
        ...
        let candidate = borrow_global_mut<ExecuteCapabilityCandidateV2<CollateralType>>(@dexlyn);
        let (exist, idx) = vector::index_of(&candidate.execute_cap_candidate, &address_of(_host));
    ...
    public entry fun claim_admin_cap<PairType, CollateralType>(
        ...
        let candidate =
            borrow_global_mut<AdminCapabilityCandidate<PairType, CollateralType>>(@dexlyn);
    ...
    public entry fun burn_execute_cap<PairType, CollateralType>(
        ...
        // If target_address is @dexlyn, the modules may no longer be available
        // Executor can remove its own ExecuteCapability, or admin can remove executor's ExecuteCapabilityStore.
        assert!(target_address != @dexlyn &&
            (target_address == address_of(_host) || address_of(_host) == @dexlyn), E_NOT_AUTHORIZED);
    ...
    public entry fun execute_order_all<PairType, CollateralType>(
        ...
        trading::execute_order_all<PairType, CollateralType>(_executor, index_price, proof_bytes, execute_cap);
    }
    ...
    public entry fun execute_order_all_v2<PairType, CollateralType>(
        ...
        trading::execute_order_all_v2<PairType, CollateralType>(_executor, index_price, proof_bytes, execute_cap);
    }
    ...
    public entry fun execute_order<PairType, CollateralType>(
        ...
        let execute_cap =
            &borrow_global<ExecuteCapabilityStore<PairType, CollateralType>>(address_of(_executor)).execute_cap;
        trading::execute_order<PairType, CollateralType>(_executor, _order_id, index_price, proof_bytes, execute_cap);
    }
    ...
    public entry fun execute_order_v2<PairType, CollateralType>(
        ...
        let execute_cap =
            &borrow_global<ExecuteCapabilityStoreV2<CollateralType>>(address_of(_executor)).execute_cap;
        trading::execute_order_v2<PairType, CollateralType>(_executor, _order_id, index_price, proof_bytes, execute_cap);
    }
    ...
    public entry fun execute_exit_position<PairType, CollateralType>(
        ...
        let execute_cap =
            &borrow_global<ExecuteCapabilityStore<PairType, CollateralType>>(address_of(_executor)).execute_cap;
        trading::execute_exit_position<PairType, CollateralType>(_executor, _user, _is_long, index_price, proof_bytes, execute_cap);
    }
    ...
    public entry fun execute_exit_position_v2<PairType, CollateralType>(
        ...
        let execute_cap =
            &borrow_global<ExecuteCapabilityStoreV2<CollateralType>>(address_of(_executor)).execute_cap;
        trading::execute_exit_position_v2<PairType, CollateralType>(_executor, _user, _is_long, index_price, proof_bytes, execute_cap);
    }
    ...
    fun T_initialize(host: &signer, supra_framework: &signer) {
        ...
        set_time_has_started_for_testing(supra_framework);
